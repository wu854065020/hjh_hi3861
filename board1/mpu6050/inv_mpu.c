...
#define MOTION_DRIVER_TARGET_MSP430		//定义驱动部分,采用MSP430的驱动(移植到Hi3861)
...
u8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
        ...
        accel_sens = 0; //把加速度计的自检去掉，上电后稳定即是当前水平坐标系角度
		accel[0] *= accel_sens;
		accel[1] *= accel_sens;
		accel[2] *= accel_sens;
...
#define PI               3.14
#define DEGREES           180
float atan2_user(float atan2_y, float atan2_x)
{
    if (atan2_x > 0) {
        return atan(atan2_y / atan2_x);
    } else if (atan2_x < 0 && atan2_y >= 0) {
        return atan(atan2_y / atan2_x) + PI;
    } else if (atan2_x < 0 && atan2_y < 0) {
        return atan(atan2_y / atan2_x) - PI;
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*roll  = atan2_user(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
		*yaw   = atan2_user(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
	}else return 2;
	return 0;
}
