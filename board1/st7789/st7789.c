...
void ST7789_WriteCommand(uint8_t cmd)
{
	ST7789_Select();
	ST7789_DC_Clr();
	hi_spi_host_write(ST7789_SPI_PORT, &cmd, sizeof(cmd));
	ST7789_UnSelect();
}
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	while (buff_size > 0) {
		...
		#else
			hi_spi_host_write(ST7789_SPI_PORT, buff, chunk_size);
		#endif
		buff += chunk_size;
...
static void ST7789_WriteSmallData(uint8_t data)
{
	ST7789_Select();
	ST7789_DC_Set();
	hi_spi_host_write(ST7789_SPI_PORT, &data, sizeof(data));
	ST7789_UnSelect();
}
 ...
 * @return none
 */
void ST7789_DrawImageU8(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *data)
{
	if ((x >= ST7789_WIDTH) || (y >= ST7789_HEIGHT))
		return;
	if ((x + w - 1) >= ST7789_WIDTH)
...
void ST7789_WriteString(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor)
{
	ST7789_Select();
	while (*str) {
		if (x + font.width - 1 >= ST7789_WIDTH) {
			x = 0;
			y += font.height;
			if (y + font.height - 1 >= ST7789_HEIGHT) {
				break;
			}
 ...
 * @brief Write a hanzi
 * @param  x&y -> cursor of the start point.
 * @param ch -> hanzi index to write
 * @param font -> fontstyle of the string
 * @param color -> color of the hanzi
 * @param bgcolor -> background color of the char
 * @return  none
 */
void ST7789_WriteHanzi(uint16_t x, uint16_t y, char ch, HanziDef hanzi, uint16_t color, uint16_t bgcolor)
{
	uint32_t i, b, j, k;
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + hanzi.width - 1, y + hanzi.height - 1);

	for (i = 0; i < hanzi.height; i++) {
		for (k = 0; k < hanzi.width/8; k++) {
			b = hanzi.data[ch * hanzi.height * (hanzi.width / 8) + i * (hanzi.width / 8) + k];
			for (j = 0; j < 8; j++) {
				if ((b << j) & 0x80) {
					uint8_t data[] = {color >> 8, color & 0xFF};
					ST7789_WriteData(data, sizeof(data));
...
void ST7789_WriteHanziStr(uint16_t x, uint16_t y, char ch, uint16_t len, HanziDef hanzi, uint16_t color, uint16_t bgcolor)
{
	while (len > 0) {
		len--;
		if (x + hanzi.width - 1 >= ST7789_WIDTH) {
			x = 0;
			y += hanzi.height;
			if (y + hanzi.height >= ST7789_HEIGHT) {
				break;
			}
		}
		ST7789_WriteHanzi(x, y, ch, hanzi, color, bgcolor);
		x += hanzi.width;
		ch++;
	}
}
